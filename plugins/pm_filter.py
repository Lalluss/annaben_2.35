            ...
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                ...
                if clicked == typed:
                    joelkb_creatorbeatz = await client.send_cached_media(
                        ...
                        [[
                          InlineKeyboardButton('Fast Download', url=await get_shortlink(query.message.chat.id,f"https://telegram.me/botechs_bot?start=files_{file.file_id}"))
                        ],[
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        ...
        cap = f"<b>Hᴇʏ {message.from_user.mention} Qᴜᴇʀʏ:{search}  Hᴇʀᴇ ɪs Wʜᴀᴛ I Fᴏᴜɴᴅ Iɴ Mʏ Dᴀᴛᴀʙᴀsᴇ 𝙲ʟɪᴄᴋ ᴛʜᴇ ʟᴀɴɢᴜᴀɢᴇ ʙᴜᴛᴛᴏɴ ᴀɴᴅ ꜱᴇʟᴇᴄᴛ yᴏᴜʀ ʟᴀɴɢᴜᴀɢᴇ .</b>"
    if imdb and imdb.get('poster'):
        try:
            if message.chat.id == SUPPORT_CHAT_ID:
